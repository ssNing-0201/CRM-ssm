...
<script type="text/javascript">
	...
	$(function(){
		...
		$("#saveCustomerBtn").click(function () {
			...
			//发送请求
			$.ajax({
				url: "workbench/customer/saveNewCustomer.do",
				data: {
					owner:owner,
					name:name,
					phone:phone,
				...
				success:function (data) {
					if (data.code=="1"){
						//刷新页面
						customerForPageList(1,$("#page").bs_pagination("getOption","rowsPerPage"));
						//重置表单
						$("#create-form")[0].reset();
						//关闭模态窗口
		...
		$("#deleteCustomerBtn").click(function () {
			...
			$.ajax({
				...
				success:function (data) {
					if (data.code=="1"){
						customerForPageList($("#page").bs_pagination("getOption","currentPage"),$("#page").bs_pagination("getOption","rowsPerPage"));
					}else {
						alert(data.message);
					}
				}
	...
	function customerForPageList(pageNo, pageSize) {
		//收集参数
		...
		$.ajax({
			...
			success:function (data) {
				...
				$.each(data.customerList,function (index, obj) {
					...
					str +='<td><a style="text-decoration: none; cursor: pointer;" onclick="window.location.href=\'detail.jsp\';">'+obj.name+'</a></td>';
					str +='<td>'+obj.owner+'</td>';
					str +='<td>'+obj.phone+'</td>';
				...
				$("#page").bs_pagination({
					...
					onChangePage : function(event, data){
						customerForPageList(data.currentPage , data.rowsPerPage);
					}
				})
			}
		})
